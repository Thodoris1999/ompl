...
def test_rv_state_space():
    ...
    assert "RealVectorStateType" in str(type(state)), (
        "State is not of type RealVectorStateType"
    )
    ...
    # Negative indices
    with pytest.raises(
        TypeError, match="Negative slice indices require a known dim count"
    ):
        _ = state[-1:2]
    ...
    # Size mismatch
    with pytest.raises(
        ValueError, match="Size of assignment does not match slice size"
    ):
...
def test_discrete_state_space():
    ...
    assert 0 <= state1.value <= 9, (
        f"UniformNear sample value {state1.value} out of bounds"
    )
...
def test_so2_state_space():
    ...
    assert isinstance(state.value, float), (
        "State value should be a float after sampling"
    )
    ...
    assert dist == pytest.approx(0.7, rel=1e-2), (
        f"Expected distance about 0.7, got {dist}"
    )
...
def test_time_state_space():
    ...
    assert timeSS.getMinTimeBound() == pytest.approx(0.0), (
        "Minimum time bound should be 0.0"
    )
    assert timeSS.getMaxTimeBound() == pytest.approx(10.0), (
        "Maximum time bound should be 10.0"
    )
    # Typically a time state space is considered bounded if setBounds has been called.
    assert timeSS.isBounded() == True, (
        "TimeStateSpace should be bounded after setBounds()"
    )
    ...
    assert raw_state.position >= 0.0 and raw_state.position <= 10.0, (
        "State position must be within bounds"
    )
    ...
    assert copy_dist == pytest.approx(0.0), (
        f"Expected copied state distance 0.0, got {copy_dist}"
    )
    ...
    assert 0.0 <= sampled_state.position <= 10.0, (
        f"Uniform sampled time {sampled_state.position} out of bounds"
    )
